                            ...
                            <div class="btn-group">
                                @if ($phraseCategory->userResult)
                                    <a class="btn btn-primary"
                                        href="{{ route('quiz.quizStart', $phraseCategory->id) }}">Play Again</a>
                                @else
                                    <a class="btn btn-primary"
                                        href="{{ route('quiz.quizStart', $phraseCategory->id) }}">Gia</a>
                                @endif
                                @if ($phraseCategory->userResult)
                                    <a class="btn btn-info"
                                        href="{{ route('quiz.quizReview', $phraseCategory->id) }}">Latest Quiz Review</a>
                                @else
                                    <a class="btn btn-info disabled"
                                        href="{{ route('quiz.quizReview', $phraseCategory->id) }}">Latest Quiz Review</a>
                                @endif

                                <a class="btn btn-secondary" href="{{ route('quiz.index') }}" role="button">Return to
                                    Quiz</a>
                            </div>
